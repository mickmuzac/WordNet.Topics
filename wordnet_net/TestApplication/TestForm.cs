...
namespace TestApplication
{
    public partial class TestForm : Form
    {
        ...
        public TestForm()
        {
            ...
            _wordNetEngine = new WordNetEngine(root + @"\Users\muzacm\Desktop\WordNet\WordNet\dict", false);
        ...
        private void runHypernymAlgorithm_Click(object sender, EventArgs e)
        {

            //Number of elements to narrow down to
            int numNarrowDown = 1;

            string[] tempWords = 
                { "Health & Physical Education", "Physical Education", "federal resources" };
                //{ "Engineering & Design", "Information Technology", "History-Social Science", "Career & Technical Ed", "federal resources" };
                //{ "Visual Arts", "Visual Arts & Performing Arts", "Music", "federal resources" };
                //{"3DR","Furniture","office","storage","wood cabinet"};
                //{"3DR","Toyota","Pickup","Truck","Civilian","Hilux","Tacoma","Ford","Chevy","GMC","vehicle"};
                //{ "3DR", "Building", "Buildings", "Structure", "Structures", "Hanger", "Hangers" };
                //{ "pbs", "pbs kids", "Teacher Resource", "Science Experiment", "Cat", "Behavior", "Handedness", "Parent Resource", "Experiment", "Math" };
                /*{ "Botany", "for", "Kids", "offers", "activities", "for", "learning", "how", "leaves", "change", "color", "how", "flowers", "grow", "how", "plant", "fight", 
            ...
            string[] tempArr;
            List<string> testWords = new List<string>(tempWords.Length);
            List<SynSet> allSynSets = new List<SynSet>(testWords.Count);
            char [] tempSplit = { ' ', '-', '&' };

            //Split spaces and remove doubles. Wordnet doesn't support PLURAL WORDS
            for (int i = 0; i < tempWords.Length; i++)
            {
                tempArr = tempWords[i].Split(tempSplit);

                if (tempArr.Length > 1)
                {
                    for (int g = 0; g < tempArr.Length; g++)
                    {
                            testWords.Add(tempArr[g]);
                        
                    }
                }
                else if (!testWords.Contains(tempArr[0]))
                {
                    testWords.Add(tempArr[0]);
                }
            }

            //Get the synsets of the array of words... you can choose between most common and all. I recommend all.
            for (int i = 0; i < testWords.Count; i++)
            {
                if (mostCommon.Checked)
                {
                    try
                    {
                        SynSet synset = _wordNetEngine.GetMostCommonSynSet(testWords[i], (WordNetEngine.POS)pos.SelectedItem);
                        if (synset != null)
                        {
                ...
                {
                    // get all synsets
                    try { allSynSets.AddRange(_wordNetEngine.GetSynSets(testWords[i], (WordNetEngine.POS)pos.SelectedItem).ToArray()); }
                    catch (Exception ex) { MessageBox.Show("Error:  " + ex); return; }

                    if(i == testWords.Count - 1) 
                        synSets.Items.AddRange(allSynSets.ToArray());
                }
            }

            //The relation we'll be using is the Hypernym. Hyponyms are too irrelevant and slow. There are other relations to consider.
            List<WordNetEngine.SynSetRelation> r = new List<WordNetEngine.SynSetRelation>(2);
            r.Add(LAIR.ResourceAPIs.WordNet.WordNetEngine.SynSetRelation.Hypernym);

            List<SynSet> tempArr2 = new List<SynSet>(testWords.Count * testWords.Count);
            SynSet temp;

            //Get the closest mutually reachable synset via hypernyms. Sounds like what we need...
            //The nested loop will compare each synset to every other synset in hopes of finding a relationship
            for (int i = 0; i < allSynSets.Count - 1; i++)
            {
                for (int g = i + 1; g < allSynSets.Count; g++)
                {
                    temp = allSynSets[i].GetClosestMutuallyReachableSynset(allSynSets[g], r);
                    if (temp != null)
                        tempArr2.Add(temp);
                }
            }

            //Should we automatically narrow the search down to a single topic?
            tempArr2 = tempArr2.Where(x => x.GetDepth(r) > numericUpDown1.Value).ToList();
            allSynSets = tempArr2;

            int tempLevel = (int)numericUpDown1.Value + 1;
            if (autoSimilar.Checked)
            {
                while (tempArr2.Count > numNarrowDown)
                {
                    tempArr2 = tempArr2.Where(x => x.GetDepth(r) > tempLevel).ToList();
                    tempLevel++;
                    ...
                    if (tempArr2.Count == 0)
                    {
                        ...
                        //Let's avoid that infinite loop
                        break;
                    }
                    else
                    {
            ...
            synSets.Items.Clear();
            synSets.Items.AddRange(tempArr2.ToArray());
        }
